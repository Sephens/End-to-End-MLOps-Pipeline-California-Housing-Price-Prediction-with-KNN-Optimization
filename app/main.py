...
async def lifespan(app: FastAPI):
    ...
    if not os.path.exists(MODEL_PATH):
        raise RuntimeError(
            ...
            "Run 'python pipeline/train.py' first."
        )
...
class HouseFeatures(BaseModel):
    ...
    MedInc:     float = Field(..., gt=0,  description="Median income in block group ($10k)")
...
class PredictionResponse(BaseModel):
    predicted_value_100k: float = Field(..., description="Predicted median house value in $100k")
...
def predict(features: HouseFeatures):
    ...
    return PredictionResponse(
        ...
        predicted_value_usd=f"${usd_value:,.0f}",
    )
